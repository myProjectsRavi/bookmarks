...
async function generateChapters(
    ...
    for (const bookmark of bookmarks) {
        ...
        // Create minimal content if still empty (just skip the 'No offline content' message)
        if (!content) {
            content = bookmark.description || '';
        }
...
/**
 * Generate ePub as a simple HTML zipped file (browser-compatible)
 * Using a simple approach since epub-gen-memory requires Node.js modules
 */
async function generateEpub(
    config: BookConfig,
    chapters: BookChapter[]
): Promise<Blob> {
    // Create a simple HTML eBook that can be opened in most readers
    const htmlContent = `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <p><em>by ${escapeHtml(config.author)}</em></p>
    <p>${chapters.length} bookmarks • Created with LinkHaven</p>
    
    <div class="toc">
        <h2>Table of Contents</h2>
        <ol>
            ${chapters.map((ch, i) => `<li><a href="#chapter-${i}">${escapeHtml(ch.title)}</a></li>`).join('\n            ')}
        </ol>
    </div>
    ...
    <div class="chapter" id="chapter-${i}">
        ...
        <p class="source"><a href="${escapeHtml(chapter.url)}">${escapeHtml(chapter.url)}</a></p>
        <div class="content">
            ${chapter.content ? formatContentAsHtml(chapter.content) : '<p><em>No saved content</em></p>'}
        </div>
    </div>
    `).join('')}
    
    <footer style="text-align: center; margin-top: 40px; color: #999;">
        <p>Generated by LinkHaven</p>
    </footer>
</body>
</html>
    `;
...
async function generatePdf(
    ...
    for (const chapter of chapters) {
        ...
        // Source URL (removed 'Source:' prefix for cleaner look)
        doc.setFontSize(8);
        doc.setFont('helvetica', 'italic');
        doc.setTextColor(100, 100, 100);
        doc.text(chapter.url, margin, 30 + titleLines.length * 7 + 5);
...
export function downloadBook(blob: Blob, filename: string, format: 'epub' | 'pdf'): void {
    // ePub now generates HTML for browser compatibility
    const ext = format === 'epub' ? '.html' : '.pdf';
